...
// The verifyUser function is now effectively unused, but can be left for potential future use
const verifyUser = asyncHandler(async (req, res) => {
  try {
    const { token } = req.params;
    const decoded = jwt.verify(token, process.env.JWT_SECRET);
    ...
    // This line doesn't hurt to keep, but won't change anything if already true
    user.isVerified = true; 
    await user.save();

    res.redirect(`${process.env.FRONTEND_URL}`);
  } catch (error) {
    console.error("Email verification failed (but bypassed):", error.message); // Updated log
    res.status(400).json({ message: "Invalid or expired token" });
  }
});
...
const auth0Login = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    user = await User.create({
      ...
      email: email,
      googleId,
      isVerified: true, // Auth0 users are always verified
    });
  } else if (!user.googleId) {
    // If user exists via email/password, link the Google ID
    user.googleId = googleId;
    user.isVerified = true; // Ensure they are marked verified
    await user.save();
  } else {
     // If user already exists via Google, ensure they are verified
     user.isVerified = true;
     await user.save();
  }
...
// forgotPassword still needs email sending, so it won't work on Render's free tier
const forgotPassword = asyncHandler(async (req, res) => {
  const { email } = req.body;
  const user = await User.findOne({ email });
...
module.exports = {
  registerUser,
  verifyUser, // Keep exporting it, even if unused for now
  loginUser,
  forgotPassword,
  resetPassword,
  auth0Login,
